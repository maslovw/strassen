...
*      c22 = q1+q3-q2+q6
*
* where the int indices refer to submatrices in an obvious way.
...
void print(int n, matrix C)
{
	int i, j;
	for (i = 0; i < n; ++i)
	{
		for (j = 0; j < n; ++j)
			printf("%u ", (int)C->d[i][j]);
		printf("\n");
	}
}

//void Init(int n, int src[][3], matrix dst)
//{
//	dst->d = (int**)malloc(n);
//	int i;
//	for (i = 0; i < n; ++i)
...
int Init(const char* apFileName, matrix aA)
{
	int m;
	...
	for (i = 0; i < m; ++i)
	{
		aA->d[i] = (int *)malloc((m+1) * sizeof(int));
		fread(aA->d[i], sizeof(int), m, tpFile);
	}
	fclose(tpFile);
	return m;
}

void Save(const char* apName, const matrix aA, int n)
{
	FILE* tpFile = fopen(apName, "wb");
	fwrite(&n, sizeof(n), 1, tpFile);
	fwrite(aA->p, sizeof(int), n*n, tpFile);
	fclose(tpFile);
}
...
void StrassenCTest(const char* apFileName)
{
	...
	multiply(n, &tA, &tB, &tC, &tD);
	//Init(tA, a[0], M);
	//Init(tB, b[0], M);
	//Scalar(tA, tB, tC);
	print(n, &tC);
	Save(apFileName, &tC, n);
}
